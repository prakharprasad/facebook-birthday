...
async def get_facebook_users():
    with ent.db_session:
        users = []
        results = FacebookUser.select(
            lambda p: p).order_by(FacebookUser.name)[:]
        if len(results) > 0:
            for user in results:
                users.append({
                    "uid": user.uid,
...
async def runner_execute(payload: RunnerExecutePayload):
    ...
    if output:
        ...
        raise HTTPException(
            status_code=404, detail="No users were wished, check if anyone has their birthday today.")
...
async def generate_db():
    ...
    if error:
        raise HTTPException(
            status_code=500, detail=f"Failed to generate database: {message}")
